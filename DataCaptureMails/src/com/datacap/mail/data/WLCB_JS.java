...
public class WLCB_JS extends DataRequest {
	...
	public static WLCB_JS getWLCB_JS() {
		return wlcb;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		mSource = "乌兰察布市政务服务中心-建设工程";
		int currentPage = 1;
		mCharset = "utf-8";
		while (true) {
			log.debug("当前获取第" + currentPage + "页数据");
			mUri = "http://www.wlcbzwfwzx.cn/articleView/articleView!list.action?categoryCode=jsgc&categoryId=136530&domainCode=nt&page.pageNo="
					+ currentPage;
			doGet(httpClient);
			// 如果抓取结束了，则跳出循环
			if (mIsOver == true) {
