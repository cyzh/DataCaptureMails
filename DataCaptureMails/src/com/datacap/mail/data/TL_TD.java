...
public class TL_TD extends DataRequest {
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		mSource = "通辽市公共资源交易中心-土地交易";
		int currentPage = 1;
		mCharset = "utf-8";
		while (true) {
			log.debug("当前获取第" + currentPage + "页数据");
			mUri = "http://www.tlzw.net/index/showInfoListJiaoyiNew.action?infoPid=24&infoMid=1139193556886&page="
					+ currentPage;
			doGet(httpClient);
			// 如果抓取结束了，则跳出循环
			if (mIsOver == true) {
