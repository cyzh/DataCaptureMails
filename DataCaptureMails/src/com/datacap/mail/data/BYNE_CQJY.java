 ...
 * 
 */
public class BYNE_CQJY extends DataRequest {
	private static final Logger log = Logger.getLogger(DataRequest.class);
	private static BYNE_CQJY byne_js = null;
	private String url = "http://zwzx.bynr.gov.cn/";

	private BYNE_CQJY() {
	}

	public static BYNE_CQJY getInstance() {
		if (byne_js == null) {
			byne_js = new BYNE_CQJY();
		}
		return byne_js;
	}

	public static BYNE_CQJY getBYNE_CQJY() {
		return byne_js;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		...
		while (true) {
			try {
				...
				params.add(new BasicNameValuePair("resourceCode", "cqzbgg"));
				params.add(new BasicNameValuePair("type", "cq"));
