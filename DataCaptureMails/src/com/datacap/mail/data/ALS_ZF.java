...
public class ALS_ZF extends DataRequest {
	...
	public static ALS_ZF getALS_ZF() {
		return als;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		mSource = "阿拉善公共资源交易网-政府采购";
		int currentPage = 1;
		mCharset = "utf-8";
		while (true) {
			log.debug("当前获取第" + currentPage + "页数据");
			mUri = "http://www.alsmzwfwzx.gov.cn:82/index/showInfoListByMiddle.action?infoPid=103&infoMid=1139193556887&page="
					+ currentPage;
			doGet(httpClient);
			// 如果抓取结束了，则跳出循环
			if (mIsOver == true) {
