 ...
 * 
 */
public class BYNE_JS extends DataRequest {
	private static final Logger log = Logger.getLogger(DataRequest.class);
	private static BYNE_JS byne_js = null;
	private String url = "http://zwzx.bynr.gov.cn/";

	private BYNE_JS() {
	}

	public static BYNE_JS getInstance() {
		if (byne_js == null) {
			byne_js = new BYNE_JS();
		}
		return byne_js;
	...
	public static BYNE_JS getBYNE_JS() {
		return byne_js;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		...
		while (true) {
			try {
				...
				params.add(new BasicNameValuePair("resourceCode", "jszbgg"));
				params.add(new BasicNameValuePair("type", "js"));
