 ...
 * 
 */
public class BYNE_GTZY extends DataRequest {
	private static final Logger log = Logger.getLogger(DataRequest.class);
	private static BYNE_GTZY byne_js = null;
	private String url = "http://zwzx.bynr.gov.cn/";

	private BYNE_GTZY() {
	}

	public static BYNE_GTZY getInstance() {
		if (byne_js == null) {
			byne_js = new BYNE_GTZY();
		}
		return byne_js;
	}

	public static BYNE_GTZY getBYNE_GTZY() {
		return byne_js;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		...
		while (true) {
			try {
				...
				params.add(new BasicNameValuePair("resourceCode", "gtzbgg"));
				params.add(new BasicNameValuePair("type", "gt"));
