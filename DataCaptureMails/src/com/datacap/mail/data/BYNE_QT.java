 ...
 * 
 */
public class BYNE_QT extends DataRequest {
	private static final Logger log = Logger.getLogger(DataRequest.class);
	private static BYNE_QT byne_js = null;
	private String url = "http://zwzx.bynr.gov.cn/";

	private BYNE_QT() {
	}

	public static BYNE_QT getInstance() {
		if (byne_js == null) {
			byne_js = new BYNE_QT();
		}
		return byne_js;
	...
	public static BYNE_QT getBYNE_QT() {
		return byne_js;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		...
		while (true) {
			try {
				...
				params.add(new BasicNameValuePair("resourceCode", "qtzbgg"));
				params.add(new BasicNameValuePair("type", "qt"));
