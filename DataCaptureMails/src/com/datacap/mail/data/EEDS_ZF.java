...
public class EEDS_ZF extends DataRequest {
	...
	public static EEDS_ZF getEEDS_ZF() {
		return eeds;
	}
	...
	protected void traversePage(CloseableHttpClient httpClient) {
		mSource = "鄂尔多斯公共资源交易中心-政府采购";
		int currentPage = 1;
		mCharset = "utf-8";
		while (true) {
			log.debug("当前获取第" + currentPage + "页数据");
			mUri = "http://www.ordosggzyjy.com/TPFront/zfcg/010001/?Paging="
					+ currentPage;
			doGet(httpClient);
			// 如果抓取结束了，则跳出循环
			if (mIsOver == true) {
